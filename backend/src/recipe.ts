...
async function seedDatabase() {
  ...
  const defaultRecipes = [
    ...
    // Add other recipes (Boiled Eggs, Vegan Soup, etc.) similarly
  {
    user_id: 1,
    name: 'Vegan Soup',
    category: 'Lunch',
...
for (const recipe of defaultRecipes) {
  await db.run(
    'INSERT OR IGNORE INTO recipes (user_id, name, category, ingredients, instructions, dietary_info, prep_time, cook_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
    [recipe.user_id, recipe.name, recipe.category, recipe.ingredients, recipe.instructions, recipe.dietary_info, recipe.prep_time, recipe.cook_time]
  );
}
